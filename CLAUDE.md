...
- `run_once_after_06-tailscale-serve-jellyfin.sh.tmpl` - Exposes Jellyfin over Tailscale Serve at `https://<mini>.<tailnet>.ts.net/jellyfin` — gated to `rishi-macmini-2020`. Requires the mini to already be logged into a tailnet *and* Jellyfin's `BaseUrl` to be `/jellyfin` in `~/.local/share/jellyfin/config/network.xml` (Dashboard → Networking → Base URL). Otherwise the script logs a notice and exits.
